...
func TestIntegration_RouteViews(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping integration test in short mode")
	}

	fetcher := NewRouteViews()

	// Test with Google ASN (AS15169)
	prefixes, err := fetcher.Fetch(15169)
	if err != nil {
		t.Fatalf("failed to fetch prefixes from RouteViews: %v", err)
	}
	...
	t.Logf("RouteViews fetched %d prefixes for AS15169 (Google)", len(prefixes))
}
